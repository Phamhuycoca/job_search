...
const { updateProfession, getData } = useProfession();
...
const getDetail = async (id: any) => {
    ...
    professionName.value = res.data.professionName;
}
const handleClose = () => {
    emit('close');
};
...
const saveData = async () => {
    ...
    formData.append('professionId', props.id);
    const res = await updateProfession(formData, props.id);
    if (res.success) {
