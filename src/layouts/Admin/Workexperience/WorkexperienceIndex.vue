<template>
    <div>
        ...
        <el-table :data="tableData" style="width: 100%">
            ...
            <el-table-column prop="workexperienceName" label="Kinh nghiệm làm việc" width="600" />
            <el-table-column fixed="right" label="Tùy chọn" width="150" align="center">
                <template #default="scope">
                    <el-button type="primary" size="small"
                        @click="id = scope.row.workexperienceId, dialogEdit = true">Sửa</el-button>
                    <el-button type="primary" size="small"
                        @click="dialogDelete = true, idRemove = scope.row.workexperienceId">Xóa</el-button>
                </template>
            </el-table-column>
        </el-table>
        ...
        <WorkexperienceCreate :dialog="dialog" @close="dialog = false" @loadData="loadData()" />
        <WorkexperienceUpdate :dialogEdit="dialogEdit" :id="id" @close="dialogEdit = false" @loadData="loadData()" />
...
import WorkexperienceCreate from '../Workexperience/WorkexperienceCreate.vue'
import WorkexperienceUpdate from '../Workexperience/WorkexperienceUpdate.vue'
...
const { fetchWorkexperiences, searchWorkexperiences, deleteWorkexperience } = useWorkexperience();
...
const RemoveItem = async () => {
    const res = await deleteWorkexperience(idRemove.value);
    if (res.success) {
...
const searchData = async () => {
    ...
    const data = await searchWorkexperiences();
    if (data) {
        tableData.value = data.items;
        if (data.totalItems !== undefined) {
            totalItems.value = data.totalItems;
