                ...
                <el-row :gutter="20">
                    <el-col :span="24">
                        <el-form-item label="Hình thức làm việc">
                            <el-input v-model="name" size="large" placeholder="Nhập họ tên" />
                        </el-form-item>
                    </el-col>
                </el-row>
            </el-form>
...
import { useFormofwork } from "./Services/formofwork.service";
const { updateFormofwork, getData } = useFormofwork();
...
const getDetail = async (id: any) => {
    ...
    name.value = res.data.formofworkName;
}
const handleClose = () => {
    emit('close');
};
...
const saveData = async () => {
    ...
    formData.append('formofworkName', name.value);
    formData.append('formofworkId', props.id);
    const res = await updateFormofwork(formData, props.id);
    if (res.success) {
