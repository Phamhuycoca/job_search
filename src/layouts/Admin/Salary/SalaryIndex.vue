<template>
    <div>
        ...
        <el-table :data="tableData" style="width: 100%">
            ...
            <el-table-column prop="salaryPrice" label="Mức lương" width="600" />
            <el-table-column fixed="right" label="Tùy chọn" width="150" align="center">
                <template #default="scope">
                    <el-button type="primary" size="small"
                        @click="id = scope.row.salaryId, dialogEdit = true">Sửa</el-button>
                    <el-button type="primary" size="small"
                        @click="dialogDelete = true, idRemove = scope.row.salaryId">Xóa</el-button>
                </template>
            </el-table-column>
        </el-table>
        ...
        <SalaryCreate :dialog="dialog" @close="dialog = false" @loadData="loadData()" />
        <SalaryUpdate :dialogEdit="dialogEdit" :id="id" @close="dialogEdit = false" @loadData="loadData()" />
...
import SalaryCreate from '../Salary/SalaryCreate.vue'
import SalaryUpdate from '../Salary/SalaryUpdate.vue'
...
const { fetchSalarys, searchSalarys, deleteSalary } = useSalary();
...
const RemoveItem = async () => {
    const res = await deleteSalary(idRemove.value);
    if (res.success) {
...
const searchData = async () => {
    ...
    const data = await searchSalarys();
    if (data) {
        tableData.value = data.items;
        if (data.totalItems !== undefined) {
            totalItems.value = data.totalItems;
